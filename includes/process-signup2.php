...
if (isset($_POST['action'])) { // Checking for submit action
	...
	if ($_POST['action'] == 'add') {
		...
		$agree	 = isset($_POST['agree']) ? trim($_POST['agree']) : false;
		...
		if ($email != '' && $message != '' && $agree) {
			if (preg_match($pattern, $email)) {
				...
				$messages = '';
				if ($name != '') $messages .= 'Full name: ' . $name . "\n";
				...
				mail($my_email, $subject_a, $messages, $headers_a);
				mail($email, $subject_b, $messages, $headers_b);
